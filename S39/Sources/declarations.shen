...
(put shen external-symbols
     ...
     *property-vector* @v @p @s *port* *porters* *hush* <- -> <e> == = >= > ==> /. <!> <end> $ - / * + <= 
     ...
     union unput unprofile undefmacro return type tuple? true trap-error tracked track time thaw tc? tc tl 
     tlstr tlv tail systemf synonyms system-S? symbol symbol? string->symbol sum subst string? string->n 
     stream string stinput stoutput step spy specialise snd simple-error set save str run reverse retract 
     ...
     declare datatype datatypes cn cons? cons cond concat compile cd cases call close bind bound? boolean? boolean 
     bootstrap (intern "bar!") spy? step? atom? asserta assertz assoc arity append and adjoin <-address address-> 
     absvector? absvector abort])
