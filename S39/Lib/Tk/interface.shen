...
(define shen->tcl
  String -> (let Sink    (open-absolute (value *out*) out)
...
(define ready?
  -> (empty? (read-file-absolute (value *out*))))
  
(define open-absolute
  File Direction -> (let Home    (value *home-directory*)
                         NewHome (set *home-directory* "")
                         Open    (open File Direction)
                         ReSet   (set *home-directory* Home)
                         Open))
                         
(define read-file-absolute
  File -> (let Home    (value *home-directory*)
               NewHome (set *home-directory* "")
               Read    (read-file File)
               ReSet   (set *home-directory* Home)
               Read))                                            
...
(define tcl->shen-no-hang
  -> (let Command (strip-eot (read-file-absolute (value *in*)))
          (if (empty? Command)
              skip
              (let Flush (flush)
              Command))))
...
(define flush
  -> (let Sink  (open-absolute (value *in*) out)
          Write (pr "" Sink)
          (close Sink)))
